...
MainWindow::MainWindow(QString github_repo, QString current_version,
                       QSettings &settings, QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), _settings(settings),
      _updater(this, github_repo, current_version) {
  _selectedArmorIds.fill(Armor::NOTHING);
  ui->setupUi(this);
  ...
  // File
  connect(ui->actionSave_Current_Armor, QAction::triggered, this,
          _saveCurrentSet);
  connect(ui->actionLoad_Armor_Toolbar, QAction::triggered, this,
          _loadSavedSet);
  ...
  // Options
  connect(ui->actionChange_Steam_Path, QAction::triggered, this,
          _getCustomSteamPath);
  connect(ui->actionAutoSteam, QAction::triggered, this, _setAutoSteam);
  connect(ui->actionAutomatically_Check_for_Updates, QAction::triggered, this,
          _toggleAutoUpdates);
  ...
  connect(ui->actionShow_Low_Rank_Armors, QAction::triggered, this,
          _toggleLRArmors);
  // Debug
  connect(ui->actionManually_Input_ID, QAction::triggered, this,
          _manualInputValue);
  // Help
  ...
  for (int i = 0; i < 5; ++i) {
    _sourceModels[i] = new QStandardItemModel(this);
    _proxyModels[i] = new ArmorFilterProxyModel(this);
    _proxyModels[i]->setSourceModel(_sourceModels[i]);
    _proxyModels[i]->setFilterCaseSensitivity(Qt::CaseInsensitive);
    _inputBoxes[i]->setModel(_proxyModels[i]);
    connect(_inputBoxes[i], QOverload<int>::of(&QComboBox::activated), this,
            &MainWindow::_onArmorSelectionChanged);
  }

  ui->actionShow_Low_Rank_Armors->setChecked(
  ...
  ui->actionNo_Backup_Ok->setChecked(
      _settings.value("General/NoBackupOk", true).toBool());
  ...
  if (_settings.value("General/AutoUpdates", true).toBool())
    _updater.checkForUpdates(
        true, _settings.value("General/FontBaseSize", 1.0F).toFloat());
  ...
  // Window size
  _childCache = this->centralWidget()->findChildren<QWidget *>(
      QString(), Qt::FindDirectChildrenOnly);
  for (auto c : _childCache)
    _baseGeo[c] = {c->size(), c->pos()};
  _baseGeo[ui->savedComboBox] = {ui->savedComboBox->minimumSize(),
                                 ui->savedComboBox->pos()};
...
void MainWindow::_initVersionSelector() {
  ...
  auto targetVersion =
      _settings.value("General/GameVersion", "Latest").toString();
  for (auto it = MH_Memory::versions.begin(); it != MH_Memory::versions.end();
       ++it) {
  ...
  if (_versionGroup->checkedAction() == 0) {
    LOG_ENTRY(WARNING, "Encountered unknown game version "
                           << targetVersion << " setting it to 'Latest'");
...
void MainWindow::_initLanguages() {
  ...
  auto targetLang =
      _settings.value("General/Language", "US English").toString();
  ...
  if (_langGroup->checkedAction() == 0) {
    LOG_ENTRY(WARNING, "Encountered unknown language "
                           << targetLang << " setting it to 'US English'");
...
void MainWindow::_initSearchBar() {
  QWidget *searchContainer = new QWidget(this);
  QHBoxLayout *searchLayout = new QHBoxLayout(searchContainer);
  searchLayout->setContentsMargins(10, 0, 0, 0);
  searchLayout->setSpacing(0);

  _searchBar = new QLineEdit(this);
  _searchBar->setPlaceholderText("Filter armor...");
  _searchBar->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
  searchLayout->addWidget(_searchBar);

  searchContainer->setSizePolicy(QSizePolicy::Expanding,
                                 QSizePolicy::Preferred);
  ui->toolBar->addWidget(searchContainer);
  connect(_searchBar, &QLineEdit::textChanged, this, &MainWindow::_filterArmor);
}