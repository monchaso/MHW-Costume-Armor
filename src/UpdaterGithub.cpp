...
UpdaterGithub::UpdaterGithub(QWidget *parent, const QString &repo_url,
                             const QString &current_version)
    : _parent(parent), _dia(nullptr),
...
void UpdaterGithub::checkForUpdates(bool silent, float font_size) {
  ...
  LOG_ENTRY(INFO, "Checking for updates...");
  LOG_ENTRY(INFO,
            "SSL Support: " << (QSslSocket::supportsSsl() ? "Yes" : "No"));
  LOG_ENTRY(INFO, "Build SSL Version: "
                      << QSslSocket::sslLibraryBuildVersionString());
  LOG_ENTRY(INFO,
            "Runtime SSL Version: " << QSslSocket::sslLibraryVersionString());

  if (!QSslSocket::supportsSsl()) {
    LOG_ENTRY(ERROR, "SSL support is missing. Skipping update check.");
    if (!_isSilent) {
      auto dia = new DialogWindow(
          this->_parent, "Update",
          "SSL/TLS initialization failed. Update check skipped.\nPlease ensure "
...
void UpdaterGithub::_onUpdateCheckFinished() {
  ...
  if (this->_reply->error() != QNetworkReply::NoError) {
    QString errorMsg = this->_reply->errorString();
    if (this->_reply->error() == QNetworkReply::SslHandshakeFailedError ||
        errorMsg.contains("TLS", Qt::CaseInsensitive) ||
        errorMsg.contains("SSL", Qt::CaseInsensitive)) {
      LOG_ENTRY(ERROR, "QNetwork SSL/TLS error: " << errorMsg);
    } else {
      LOG_ENTRY(ERROR, "QNetwork returned errror " << errorMsg);
    }
    if (_isSilent)
      return;
    auto dia =
        new DialogWindow(this->_parent, "Update",
  ...
  _latestVersionString =
      this->_reply->url().url().split("/", Qt::SkipEmptyParts).last();
  ...
  if (_latestVersionString.contains("major")) {
    ...
    auto dia = new DialogWindow(
        this->_parent, "Update",
        "There's an update available, but i'ts major release, please "
        "re-download the entire folder for the application again",
        Status::WARNING);
    dia->show();
    return;
  }
...
void UpdaterGithub::downloadChangelog() {
  ...
  auto url = _updatePageAddress.arg("CHANGELOG.md")
                 .replace("releases", _latestVersionString)
...
void UpdaterGithub::downloadUpdate() {
  ...
  _downloadedBinaryFile.setFileName(QCoreApplication::applicationFilePath() +
                                    ".NEW");
  ...
  auto url = _updatePageAddress.arg("download/%1/LatestUpdate.exe")
                 .arg(_latestVersionString);
  auto req = QNetworkRequest(QUrl(url));
  req.setSslConfiguration(QSslConfiguration::defaultConfiguration()); // HTTPS
  ...
  connect(this->_reply, &QNetworkReply::readyRead, this,
          this->_onNewDataDownloaded);
  connect(this->_reply, &QNetworkReply::downloadProgress, _dia,
          _dia->onUpdateDownloadProgress);
  connect(this->_reply, &QNetworkReply::finished, this,
          this->_onDownloadFinished, Qt::UniqueConnection);
}
...
void UpdaterGithub::_onDownloadFinished() {
  ...
  if (this->_reply->error() != QNetworkReply::NoError) {
    LOG_ENTRY(ERROR, "Error while downloading the update "
                         << this->_reply->errorString());
    auto dia =
        new DialogWindow(this->_parent, "Update",
